...
function getHunterKey(): string {
  ...
  if (!apiKey) {
    log.error("HUNTER_API_KEY environment variable not set");
...
export function createHunterDomainSearchTool(contactsLimit: number = 10) {
  ...
  return new DynamicStructuredTool({
    name: "people_lookup",
    description:
      `Find engineers at a company. Provide either the company domain (e.g., 'stripe.com') or company name. Returns up to ${limit} engineers with their names, titles, and emails.`,
    schema: z.object({
